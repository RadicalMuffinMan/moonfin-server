...
const Navbar = {
    ...
    createNavbar() {
        ...
        this.container.innerHTML = `
            <!-- Left section: User (native Jellyfin user button will be moved here) -->
            ...
            <div class="moonfin-navbar-center" style="background: ${overlayColor}">
                    ...
                    <button class="moonfin-nav-btn moonfin-nav-cast" data-action="cast" title="Cast">
                        <svg viewBox="0 0 24 24">
                            <path fill="currentColor" d="M1 18v3h3c0-1.66-1.34-3-3-3m0-4v2c2.76 0 5 2.24 5 5h2c0-3.87-3.13-7-7-7m0-4v2c4.97 0 9 4.03 9 9h2c0-6.08-4.93-11-11-11m20-7H3c-1.1 0-2 .9-2 2v3h2V5h18v14h-7v2h7c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2"/>
                        </svg>
                    </button>
    ...
    async loadUserData() {
        try {
            ...
            console.error('[Moonfin] Error loading user data:', error);
        }

        try {
            this.libraries = await API.getUserViews();
            if (!this.libraries) {
                console.warn('[Moonfin] No libraries returned, initializing empty array');
                this.libraries = [];
            }
            this.updateLibraries();
        } catch (error) {
            console.error('[Moonfin] Error loading libraries:', error);
            this.libraries = [];
            this.updateLibraries();
        }
    },
    ...
    updateLibraries() {
        ...
        if (!librariesContainer) {
            console.warn('[Moonfin] Libraries container not found in navbar');
            return;
        }

        if (!this.libraries || !Array.isArray(this.libraries)) {
            console.warn('[Moonfin] Libraries not loaded or invalid');
        ...
        console.log('[Moonfin] Updating libraries, found:', filteredLibraries.length);
    ...
    setupEventListeners() {
        ...
        this.moveNativeUserButton();

        this.addMoonfinSettingsLink();
    ...
    showCastMenu(button) {
        import('../components/playback/playerSelectionMenu').then((playerSelectionMenu) => {
            playerSelectionMenu.show(button);
        }).catch((err) => {
            const PlayerSelectionMenu = window.require ? window.require(['components/playback/playerSelectionMenu']) : null;
            if (PlayerSelectionMenu) {
                PlayerSelectionMenu.show(button);
            } else {
                console.error('[Moonfin] Cast menu not available:', err);
            }
        });
    },

    showSyncPlayMenu(button) {
        if (window.groupSelectionMenu) {
            window.groupSelectionMenu.show(button);
        } else {
            console.warn('[Moonfin] SyncPlay menu not available');
        }
    },

    moveNativeUserButton() {
        const nativeUserBtn = document.querySelector('.headerUserButton');
        const navbarLeft = this.container.querySelector('.moonfin-navbar-left');
        
        if (nativeUserBtn && navbarLeft) {
            navbarLeft.appendChild(nativeUserBtn);
            nativeUserBtn.classList.add('moonfin-native-user-btn');
        }
    },

    addMoonfinSettingsLink() {
        // Check if we're on the user preferences menu page
        const menuPage = document.querySelector('#myPreferencesMenuPage:not(.hide)');
        if (!menuPage) return;

        // Check if link already exists
        if (document.querySelector('#moonfinSettingsLink')) return;

        // Find the first vertical section (after Controls)
        const menuContainer = menuPage.querySelector('.verticalSection');
        if (!menuContainer) return;

        // Create the Moonfin settings link
        const moonfinLink = document.createElement('a');
        moonfinLink.id = 'moonfinSettingsLink';
        moonfinLink.setAttribute('is', 'emby-linkbutton');
        moonfinLink.className = 'button-link emby-button';
        moonfinLink.href = '#';
        moonfinLink.style.cssText = 'padding: 1em; display: block; text-decoration: none;';
        
        moonfinLink.innerHTML = `
            <div class="listItem listItem-border" style="padding: 0;">
                <span class="listItemIcon material-icons palette" aria-hidden="true">palette</span>
        ...
        moonfinLink.addEventListener('click', (e) => {
            e.preventDefault();
            SettingsPanel.toggle();
        });

        // Append to the menu container
        menuContainer.appendChild(moonfinLink);
    },
...
const MediaBar = {
    ...
    updateVisibility() {
        ...
        const isHomePage = hash === '#/home.html' || 
                          hash === '#/home' || 
                          (hash === '' && (pathname === '/' || pathname === '/web/' || pathname === '/web/index.html'));
